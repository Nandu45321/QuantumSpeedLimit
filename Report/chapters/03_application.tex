...
For states on the Bloch sphere equator, the generalized Bures angle simplifies to:
\begin{equation}
...
Figure \ref{fig:mt_bound} shows equatorial states ($\theta = \pi/2$) evolve fastest, reaching an orthogonal state ($\alpha = \pi/2$) at $\omega_0 t = \pi$. Other states never reach orthogonality, staying below the maximum bound. The periodic variation of $\alpha(t)$ reflects unitary evolution: the state precesses around the z-axis, reaching maximum distinguishability at half the period ($\omega_0 t = \pi$) before rotating back to its start, completing the cycle.
